using System;
using System.Collections.Generic;
using System.Configuration;
using System.Data;
using System.Data.SqlClient;
using System.Linq;
using System.Web;
using System.Web.Mvc;
using Job_Portal.Models;

namespace Job_Portal.Controllers
{
    public class CandidatesAppliedController : Controller
    {
		JOB_PORTALEntities dbobj = new JOB_PORTALEntities();
        // GET: CandidatesApplied
        public ActionResult CandidatesApplied_Pageload()
        {
			return View(Getdata());
		}

		public CandidatesApplied Getdata()
		{
			using (var con = new SqlConnection(ConfigurationManager.ConnectionStrings["Test"].ConnectionString))
			{
				using (SqlCommand cmd = new SqlCommand("sp_candidates", con))
				{
					cmd.CommandType = CommandType.StoredProcedure;
					cmd.Parameters.AddWithValue("@cid", Convert.ToInt32(Session["cid"]));
					con.Open();
					SqlDataReader dr = cmd.ExecuteReader();
					var jobs = new CandidatesApplied();
					while (dr.Read())
					{
						var job = new Candidates();
						job.Job_Id = Convert.ToInt32(dr["Job_Id"]);
						job.Job_Title = dr["Job_Title"].ToString();
						job.Job_Vacancy= Convert.ToInt32(dr["Job_Vacancy"]);
						job.Job_Description = dr["Job_Description"].ToString();
						job.Job_Type = dr["Job_Type"].ToString();
						job.Job_Status = dr["Job_Status"].ToString();
						job.User_Id= Convert.ToInt32(dr["User_Id"]);
						job.Date = DateTime.Parse(dr["Date"].ToString());
						job.Resume = dr["Resume"].ToString();
						jobs.selectjob.Add(job);
					}
					con.Close();
					return jobs;
				}
			}
		}
    }
}